...
%   stimCat = contains(comparisonCategoryLabel{group_i}, 'ing') + 1;
  stimLabelCount = length(comparisonCategoryLabel{group_i});
  ...
  explainedVar = nan(size(spikesPerLabel,3), size(spikesPerLabel,2), 3);
  ...
  for ep_i = 1:size(spikesPerLabel,2)
    for unit_i = 1:size(spikesPerLabel,3)
      ...
      % Perform Test
%       [pMat(unit_i, ep_i), ~, ~, ~] = anovan(spikeCounts, labelNum, 'display', 'off', 'varnames',{'Category'}, 'alpha', alpha);
      [pMat(unit_i, ep_i, :), B, ~, ~] = anovan(spikeCounts, [{stimCatLabels(socLabelNum)}; {stimLabels(labelNum)}; {eyeLabels(labelEye)}], 'model', 'interaction', 'nested', [0 0 0; 1 0 0; 0 0 0], 'display', 'off', 'varnames',{'SocialLabel', 'Category', 'Eye'}, 'alpha', alpha);
...
%         multTable = multcompare(C, 'Display','off');
%         multTable = [double(multTable(:,1)), double(multTable(:,2)), multTable(:,4), multTable(:,end)];
%         [~, ai] = sort(multTable(:, end));
%         multTable = multTable(ai,:);
%         fprintf('Explained Variance Cat %s/%s = %s \n', num2str(B{2, 2}, 2), num2str(round(B{4, 2})), num2str(B{2, 2}/B{4, 2}, 1))
%         fprintf('Explained Variance Eye %s/%s = %s \n', num2str(B{3, 2}, 2), num2str(round(B{4, 2})), num2str(B{3, 2}/B{4, 2}, 1))
