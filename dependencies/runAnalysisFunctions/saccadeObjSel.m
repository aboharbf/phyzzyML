function selTable = saccadeObjSel(spikesByEventBinned, eyeBehStatsByStim, psthParams, taskData, catIndStruct, selTable)
...
if socSaccOnly
  ...
  saccadeTimesTmp = [eyeBehStatsByStim{socInd}]';
else
...
for chan_i = 1:chanCount
  for unit_i = 1:length(spikesByUnitBinned{chan_i})
    ...
    for sacc_i = 1:saccadeCount
      ...
      % Collect all the saccade spikes within a specific window
      ...
      % Collect the spikes into a structure to make into a PSTH.
      saccSlice = zeros(1, psthPreSacc + psthPostSacc+1);
      spikeBin = saccadetimes(1, sacc_i)-psthPreSacc:saccadetimes(1, sacc_i)+psthPostSacc; % use method in paper
      
      % Shift the sampling window so it doesn't run under or over the
      % data available
      if any(spikeBin == 0)
        startInd = find(spikeBin > 0,1);
        spikeBin = spikeBin(spikeBin > 0);
      else
        startInd = 1;
      end
      
      if any(spikeBin > binCount)
        endInd = find(spikeBin > binCount, 1);
        spikeBin = find(spikeBin(spikeBin > binCount),1);
      else
        endInd = length(saccSlice);
      end
      
      saccSlice(startInd:endInd) = spikesByUnitBinned{chan_i}{unit_i}(trialInds(sacc_i), spikeBin);
      dirInd = circleInRadians == saccadeDir(sacc_i);
      saccPSTH{dirInd} = [saccPSTH{dirInd}; saccSlice];
...
selTable.(sprintf('saccObj_%s_pVal', comparisonType{comparison_i})) = unitSelVec;